...
/**
 * For original documentation, i.e. DecimalFormat
 * 
...
public class Fx702pDecimalFormat extends NumberFormat
{
  /**
   ...
   * <p>
   * To obtain standard formats for a given locale, use the factory methods on NumberFormat such as getNumberInstance. These factories will return the most
   ...
   */
  public Fx702pDecimalFormat(String pattern)
  {
   ...
   * <p>
   * To obtain standard formats for a given locale, use the factory methods on NumberFormat such as getInstance or getCurrencyInstance. If you need only minor
   * adjustments to a standard format, you can modify the format returned by a NumberFormat factory method.
   * 
   * @param pattern
   *          a non-localized pattern string
   * @param symbols
   ...
   */
  public Fx702pDecimalFormat(String pattern, DecimalFormatSymbols symbols)
  {
    // Always applyPattern after the symbols are set
   ...
   * @param toAppendTo
   *          the <code>StringBuffer</code> to which the formatted text is to be appended
   * @param pos
   ...
   * @exception NullPointerException
   *              if <code>toAppendTo</code> or <code>pos</code> is null
  ...
  private StringBuffer format(double number, StringBuffer result, FieldDelegate delegate)
  {
    ...
    /*
     * Detecting whether a double is negative is easy with the exception of the value -0.0. This is a double which has a zero mantissa (and exponent), but a
     * negative sign bit. It is semantically distinct from a zero with a positive sign bit, and this distinction is important to certain kinds of computations.
     * However, it's a little tricky to detect, since (-0.0 == 0.0) and !(-0.0 < 0.0). How then, you may ask, does it behave distinctly from +0.0? Well,
     * 1/(-0.0) == -Infinity. Proper detection of -0.0 is needed to deal with the issues raised by bugs 4106658, 4106667, and 4147706. Liu 7/6/98.
     */
  ...
  /**
   * Formats an Object producing an <code>AttributedCharacterIterator</code>. You can use the returned <code>AttributedCharacterIterator</code> to build the
   * resulting String, as well as to determine information about the resulting String.
   * <p>
   * Each attribute key of the AttributedCharacterIterator will be of type <code>NumberFormat.Field</code>, with the attribute value being the same as the
   * attribute key.
   * 
   * @exception NullPointerException
   *              if obj is null.
  ...
  private StringBuffer subformat(StringBuffer result, FieldDelegate delegate, boolean isNegative, boolean isInteger, int maxIntDigits, int minIntDigits, int maxFraDigits, int minFraDigits)
  {
    ...
    /*
     * Per bug 4147706, DecimalFormat must respect the sign of numbers which format as zero. This allows sensible computations and preserves relations such as
     * signum(1/x) = signum(x), where x is +Infinity or -Infinity. Prior to this fix, we always formatted zero values as if they were positive. Liu 7/6/98.
     */
    if (digitList.isZero())
    {
  ...
  /**
   * Appends the String <code>string</code> to <code>result</code>. <code>delegate</code> is notified of all the <code>FieldPosition</code>s in
   * <code>positions</code>.
   * <p>
   * If one of the <code>FieldPosition</code>s in <code>positions</code> identifies a <code>SIGN</code> attribute, it is mapped to <code>signAttribute</code>.
   * This is used to map the <code>SIGN</code> attribute to the <code>EXPONENT</code> attribute as necessary.
   * <p>
   ...
   * <p>
   * The method attempts to parse text starting at the index given by <code>pos</code>. If parsing succeeds, then the index of <code>pos</code> is updated to
   * the index after the last character used (parsing does not necessarily use all characters up to the end of the string), and the parsed number is returned.
   * The updated <code>pos</code> can be used to indicate the starting point for the next call to this method. If an error occurs, then the index of
   * <code>pos</code> is not changed, the error index of <code>pos</code> is set to the index of the character where the error occurred, and null is returned.
   * <p>
   * The subclass returned depends on the value of {@link #isParseBigDecimal} as well as on the string being parsed.
   * <ul>
   * <li>If <code>isParseBigDecimal()</code> is false (the default), most integer values are returned as <code>Long</code> objects, no matter how they are
   * written: <code>"17"</code> and <code>"17.000"</code> both parse to <code>Long(17)</code>. Values that cannot fit into a <code>Long</code> are returned as
   * <code>Double</code>s. This includes values with a fractional part, infinite values, <code>NaN</code>, and the value -0.0. <code>DecimalFormat</code> does
   * <em>not</em> decide whether to return a <code>Double</code> or a <code>Long</code> based on the presence of a decimal separator in the source string. Doing
   * so would prevent integers that overflow the mantissa of a double, such as <code>"-9,223,372,036,854,775,808.00"</code>, from being parsed accurately.
   * <p>
   * Callers may use the <code>Number</code> methods <code>doubleValue</code>, <code>longValue</code>, etc., to obtain the type they want.
   * <li>If <code>isParseBigDecimal()</code> is true, values are returned as <code>BigDecimal</code> objects. The values are the ones constructed by
   * {@link java.math.BigDecimal#BigDecimal(String)} for corresponding strings in locale-independent format. The special cases negative and positive infinity
   * and NaN are returned as <code>Double</code> instances holding the values of the corresponding <code>Double</code> constants.
   * </ul>
   * <p>
   * <code>DecimalFormat</code> parses all Unicode characters that represent decimal digits, as defined by <code>Character.digit()</code>. In addition,
   * <code>DecimalFormat</code> also recognizes as digits the ten consecutive characters starting with the localized zero digit defined in the
  ...
  public Number parse(String text, ParsePosition pos)
  {
    ...
    else
    {
      ...
      if (multiplier != 1 && gotDouble)
      {
        ...
        gotDouble = ((doubleResult != longResult) || (doubleResult == 0.0 && 1 / doubleResult < 0.0)) && !isParseIntegerOnly();
      }
  ...
  private final boolean subparse(String text, ParsePosition parsePosition, String positivePrefix, String negativePrefix, DigitList digits, boolean isExponent, boolean status[])
  {
    ...
    else
    {
      ...
      for (; position < text.length(); ++position)
      {
        ...
        /*
         * We recognize all digit ranges, not only the Latin digit range '0'..'9'. We do so by using the Character.digit() method, which converts a valid
         * Unicode digit to the range 0..9.
         * 
         * The character 'ch' may be a digit. If so, place its value from 0 to 9 in 'digit'. First try using the locale digit, which may or MAY NOT be a
         * standard Unicode digit range. If this fails, try using the standard Unicode digit ranges by calling Character.digit(). If this also fails, digit will
    ...
    if (!isExponent)
    {
      ...
      parsePosition.index = position + (gotPositive ? positiveSuffix.length() : negativeSuffix.length()); // mark
                                                                                                          // success!
    }
    else
    {
  ...
  /**
   * Returns the FieldPositions of the fields in the prefix used for positive numbers. This is not used if the user has explicitly set a positive prefix via
   * <code>setPositivePrefix</code>. This is lazily created.
   * 
  ...
  /**
   * Returns the FieldPositions of the fields in the prefix used for negative numbers. This is not used if the user has explicitly set a negative prefix via
   * <code>setNegativePrefix</code>. This is lazily created.
   * 
   * @return FieldPositions in positive prefix
   */
  ...
  /**
   * Returns the FieldPositions of the fields in the suffix used for positive numbers. This is not used if the user has explicitly set a positive suffix via
   * <code>setPositiveSuffix</code>. This is lazily created.
   * 
   * @return FieldPositions in positive prefix
   */
  ...
  /**
   * Returns the FieldPositions of the fields in the suffix used for negative numbers. This is not used if the user has explicitly set a negative suffix via
   * <code>setNegativeSuffix</code>. This is lazily created.
   * 
   * @return FieldPositions in positive prefix
   */
  ...
  /**
   * Sets the multiplier for use in percent, per mille, and similar formats. For a percent format, set the multiplier to 100 and the suffixes to have '%' (for
   * Arabic, use the Arabic percent sign). For a per mille format, set the multiplier to 1000 and the suffixes to have '&#92;u2030'.
   * 
   * <P>
  ...
  /**
   * Return the grouping size. Grouping size is the number of digits between grouping separators in the integer portion of a number. For example, in the number
  ...
  /**
   * Set the grouping size. Grouping size is the number of digits between grouping separators in the integer portion of a number. For example, in the number
   * "123,456.78", the grouping size is 3. <br>
  ...
  public Object clone()
  {
    try
    {
      Fx702pDecimalFormat other = (Fx702pDecimalFormat)super.clone();
  ...
  public boolean equals(Object obj)
  {
    ...
    Fx702pDecimalFormat other = (Fx702pDecimalFormat)obj;
  ...
  /**
   * Expand the affix pattern strings into the expanded affix strings. If any affix pattern string is null, do not expand it. This method should be called any
   * time the symbols or the affix patterns change in order to keep the expanded affix strings up to date.
   */
  ...
  /**
   * Expand an affix pattern into an affix string. All characters in the pattern are literal unless prefixed by QUOTE. The following characters after QUOTE are
   * recognized: PATTERN_PERCENT, PATTERN_PER_MILLE, PATTERN_MINUS, and CURRENCY_SIGN. If CURRENCY_SIGN is doubled (QUOTE + CURRENCY_SIGN + CURRENCY_SIGN), it
   * is interpreted as an ISO 4217 currency code. Any other character after a QUOTE represents itself. QUOTE must be followed by another character; QUOTE may
   * not occur by itself at the end of the pattern.
   * 
   * @param pattern
  ...
  /**
   * Expand an affix pattern into an array of FieldPositions describing how the pattern would be expanded. All characters in the pattern are literal unless
   * prefixed by QUOTE. The following characters after QUOTE are recognized: PATTERN_PERCENT, PATTERN_PER_MILLE, PATTERN_MINUS, and CURRENCY_SIGN. If
   * CURRENCY_SIGN is doubled (QUOTE + CURRENCY_SIGN + CURRENCY_SIGN), it is interpreted as an ISO 4217 currency code. Any other character after a QUOTE
   * represents itself. QUOTE must be followed by another character; QUOTE may not occur by itself at the end of the pattern.
   * 
   * @param pattern
   *          the non-null, possibly empty pattern
  ...
  /**
   * Appends an affix pattern to the given StringBuffer, quoting special characters as needed. Uses the internal affix pattern, if that exists, or the literal
   * affix, if the internal affix pattern is null. The appended string will generate the same affix pattern (or literal affix) when passed to toPattern().
   * 
   * @param buffer
   ...
   * @param expAffix
   *          a corresponding expanded affix, such as positivePrefix. Ignored unless affixPattern is null. If affixPattern is null, then expAffix is appended as
   *          a literal affix.
   * @param localized
  ...
  private String toPattern(boolean localized)
  {
    ...
    for (int j = 1; j >= 0; --j)
    {
      ...
      if (j == 1)
      {
        ...
        negativeSuffix.equals(positiveSuffix)) || (negSuffixPattern != null && negSuffixPattern.equals(posSuffixPattern)))
        {
          if ((negPrefixPattern != null && posPrefixPattern != null && negPrefixPattern.equals("'-" + posPrefixPattern)) || (negPrefixPattern == posPrefixPattern && // n
                                                                                                                                                                     // ==
                                                                                                                                                                     // p
                                                                                                                                                                     // ==
                                                                                                                                                                     // null
  ...
  /**
   * Apply the given pattern to this Format object. A pattern is a short-hand specification for the various formatting properties. These properties can also be
   * changed individually through the various setter methods.
   * <p>
   * There is no limit to integer digits set by this routine, since that is the typical end-user desire; use setMaximumInteger if you want to set a real value.
  ...
  /**
   * Apply the given pattern to this Format object. The pattern is assumed to be in a localized notation. A pattern is a short-hand specification for the
   * various formatting properties. These properties can also be changed individually through the various setter methods.
   * <p>
   * There is no limit to integer digits set by this routine, since that is the typical end-user desire; use setMaximumInteger if you want to set a real value.
   * For negative numbers, use a second pattern, separated by a semicolon
   * <P>
  ...
  private void applyPattern(String pattern, boolean localized)
  {
    ...
    for (int j = 1; j >= 0 && start < pattern.length(); --j)
    {
      ...
      // ".###" -> ".0##".
      /*
       * We allow patterns of the form "####" to produce a zeroDigitCount of zero (got that?); although this seems like it might make it possible for format()
       * to produce empty strings, format() checks for this condition and outputs a zero digit in this situation. Having a zeroDigitCount of zero yields a
       * minimum integer digits of zero, which allows proper round-trip patterns. That is, we don't want "#" to become "#0" when toPattern() is called (even
      ...
      if (j == 1)
      {
        ...
        /*
         * The effectiveDecimalPos is the position the decimal is at or would be at if there is no decimal. Note that if decimalPos<0, then digitTotalCount ==
  ...
  /**
   * Sets the maximum number of digits allowed in the integer portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of <code>newValue</code> and 309 is used. Negative input values are replaced with 0.
   * 
  ...
  /**
   * Sets the minimum number of digits allowed in the integer portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of <code>newValue</code> and 309 is used. Negative input values are replaced with 0.
   * 
  ...
  /**
   * Sets the maximum number of digits allowed in the fraction portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of <code>newValue</code> and 340 is used. Negative input values are replaced with 0.
   * 
  ...
  /**
   * Sets the minimum number of digits allowed in the fraction portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of <code>newValue</code> and 340 is used. Negative input values are replaced with 0.
   * 
  ...
  /**
   * Gets the maximum number of digits allowed in the integer portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of the return value and 309 is used.
   * 
  ...
  /**
   * Gets the minimum number of digits allowed in the integer portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of the return value and 309 is used.
   * 
  ...
  /**
   * Gets the maximum number of digits allowed in the fraction portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of the return value and 340 is used.
   * 
  ...
  /**
   * Gets the minimum number of digits allowed in the fraction portion of a number. For formatting numbers other than <code>BigInteger</code> and
   * <code>BigDecimal</code> objects, the lower of the return value and 340 is used.
   * 
  ...
  /**
   * Gets the currency used by this decimal format when formatting currency values. The currency is obtained by calling {@link DecimalFormatSymbols#getCurrency
  ...
  /**
   * Sets the currency used by this number format when formatting currency values. This does not update the minimum or maximum number of fraction digits used by
   * the number format. The currency is set by calling {@link DecimalFormatSymbols#setCurrency DecimalFormatSymbols.setCurrency} on this number format's
   * symbols.
   * 
   * @param currency
   ...
   * 
   * <p>
   * This pattern is expanded by the method <code>expandAffix()</code> to <code>positivePrefix</code> to update the latter to reflect changes in
   * <code>symbols</code>. If this variable is <code>null</code> then <code>positivePrefix</code> is taken as a literal value that does not change when
   * <code>symbols</code> changes. This variable is always <code>null</code> for <code>DecimalFormat</code> objects older than stream version 2 restored from
   * stream.
   * 
   * @serial
   * @since 1.3
  ...
  /**
   * The number of digits between grouping separators in the integer portion of a number. Must be greater than 0 if <code>NumberFormat.groupingUsed</code> is
   * true.
   * 
   * @serial
   * @see #getGroupingSize
  ...
  /**
   * The <code>DecimalFormatSymbols</code> object used by this format. It contains the symbols used to format numbers, e.g. the grouping separator, decimal
   * separator, and so on.
   * 
   * @serial
  ...
  /**
   * The maximum number of digits allowed in the integer portion of a <code>BigInteger</code> or <code>BigDecimal</code> number.
   * <code>maximumIntegerDigits</code> must be greater than or equal to <code>minimumIntegerDigits</code>.
   * 
   * @serial
  ...
  /**
   * The minimum number of digits allowed in the integer portion of a <code>BigInteger</code> or <code>BigDecimal</code> number.
   * <code>minimumIntegerDigits</code> must be less than or equal to <code>maximumIntegerDigits</code>.
   * 
   * @serial
  ...
  /**
   * The maximum number of digits allowed in the fractional portion of a <code>BigInteger</code> or <code>BigDecimal</code> number.
   * <code>maximumFractionDigits</code> must be greater than or equal to <code>minimumFractionDigits</code>.
   * 
   * @serial
  ...
  /**
   * The minimum number of digits allowed in the fractional portion of a <code>BigInteger</code> or <code>BigDecimal</code> number.
   * <code>minimumFractionDigits</code> must be less than or equal to <code>maximumFractionDigits</code>.
   * 
   * @serial
  ...
  /**
   * The CURRENCY_SIGN is the standard Unicode symbol for currency. It is used in patterns and substituted with either the currency symbol, or if it is doubled,
   * with the international currency symbol. If the CURRENCY_SIGN is seen in a pattern, then the decimal separator is replaced with the monetary decimal
   * separator.
   * 
