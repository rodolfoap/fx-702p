...
import com.fx702p.interpreters.formatters.AttributedCharacterIterator.Attribute;
...
public class AttributedString
{
  ...
  // We allocate them in chunks of 10 elements so we don't have to allocate all
  // the time.
   ...
   *              if the text has length 0 and the attributes parameter is not
   *              an empty Map (attributes cannot be applied to a 0-length
   *              range).
   */
   ...
   * any attributes wrapped by an Annotation object are discarded for a subrange
   * of the original attribute range.
   * 
   * @param text
   *          The text for this attributed string.
   * @param beginIndex
   ...
   *              if <code>text</code> is null.
   * @exception IllegalArgumentException
   *              if the subrange given by beginIndex and endIndex is out of the
   *              text range.
   ...
   * that match the given attributes will be incorporated into the instance. If
   * the given range produces an empty text, all attributes will be discarded.
   * Note that any attributes wrapped by an Annotation object are discarded for
   * a subrange of the original attribute range.
   * 
   * @param text
   *          The text for this attributed string.
   * @param beginIndex
  ...
  /**
   * Ensures there is a run break at offset, returning the index of the run. If
   * this results in splitting a run, two things can happen:
   * <ul>
   * <li>If copyAttrs is true, the attributes from the existing run will be
   * placed in both of the newly created runs.
   * <li>If copyAttrs is false, the attributes from the existing run will NOT be
   * copied to the run to the right (>= offset) of the break, but will exist on
   * the run to the left (< offset).
   * </ul>
   */
  private final int ensureRunBreak(int offset, boolean copyAttrs)
  {
    ...
    // make copies of the attribute information of the old run that the new one
    // used to be part of
    // use temporary variables so things remain consistent in case of an
    // exception
  ...
  private void addAttributeRunData(Attribute attribute, Object value, int beginRunIndex, int endRunIndex)
  {
    ...
    {
      int keyValueIndex = -1; // index of key and value in our vectors; assume
   ...
   * @exception IllegalArgumentException
   *              if beginIndex is less then 0, endIndex is greater than the
   *              length of the string, or beginIndex is greater than endIndex.
   */
  ...
  // gets an attribute value, but returns an annotation only if it's range does
  // not extend outside the range beginIndex..endIndex
  ...
  // returns whether all specified attributes have equal values in the runs with
  // the given indices
  ...
  final private class AttributedStringIterator implements AttributedCharacterIterator
  {
    ...
    // we don't synchronize on the iterator, assuming that an iterator is only
    // used in one thread.
    // we do synchronize access to the AttributedString however, since it's more
    // likely to be shared between threads.
    ...
    {
      // ??? This should screen out attribute keys that aren't relevant to the
      // client
