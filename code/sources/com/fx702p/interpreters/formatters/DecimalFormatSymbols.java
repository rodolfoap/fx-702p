 ...
 * <code>DecimalFormatSymbols</code> from its locale data. If you need to change
...
public class DecimalFormatSymbols implements Cloneable, Serializable
{
  ...
  /**
   * Create a DecimalFormatSymbols object for the default locale. This
   * constructor can only construct instances for the locales supported by the
   * Java runtime environment, not for those supported by installed
   * {@link java.text.spi.DecimalFormatSymbolsProvider
   * DecimalFormatSymbolsProvider} implementations. For full locale coverage,
   * use the {@link #getInstance(Locale) getInstance} method.
   */
  ...
  /**
   * Create a DecimalFormatSymbols object for the given locale. This constructor
   * can only construct instances for the locales supported by the Java runtime
   ...
   * This method provides access to <code>DecimalFormatSymbols</code> instances
   * for locales supported by the Java runtime itself as well as for those
   * supported by installed {@link java.text.spi.DecimalFormatSymbolsProvider
   ...
   * Currency.getInstance}), this also sets the currency attribute to the
   * corresponding Currency instance and the currency symbol attribute to the
   * currency's symbol in the DecimalFormatSymbols' locale. If the currency code
   * is not valid, then the currency attribute is set to null and the currency
   * symbol attribute is not modified.
   * 
   * @see #setCurrency
  ...
  /**
   * Sets the currency of these DecimalFormatSymbols. This also sets the
   * currency symbol attribute to the currency's symbol in the
   * DecimalFormatSymbols' locale, and the international currency symbol
  ...
  private void initialize(Locale locale)
  {
    ...
    java.text.DecimalFormatSymbols realDecimalFormatSymbols = new java.text.DecimalFormatSymbols(locale);


    decimalSeparator = realDecimalFormatSymbols.getDecimalSeparator();
    groupingSeparator = realDecimalFormatSymbols.getGroupingSeparator();
    patternSeparator = realDecimalFormatSymbols.getPatternSeparator();
    percent = realDecimalFormatSymbols.getPercent();
    zeroDigit = realDecimalFormatSymbols.getZeroDigit(); // different for
                                                         // Arabic,etc.
    digit = realDecimalFormatSymbols.getDigit();
    minusSign = realDecimalFormatSymbols.getMinusSign();
    exponential = 'E';
    exponentialSeparator = "" + exponential;

    perMill = realDecimalFormatSymbols.getPerMill();
    infinity = realDecimalFormatSymbols.getInfinity();
    NaN = realDecimalFormatSymbols.getNaN();
    ...
    if (currency != null)
    {
      currencySymbol = realDecimalFormatSymbols.getCurrencySymbol();
    }
    else
    {
      // default values
   ...
   * in older serial versions, and initializes non-serializable fields. If
   * <code>serialVersionOnStream</code> is less than 1, initializes
   * <code>monetarySeparator</code> to be the same as
   * <code>decimalSeparator</code> and <code>exponential</code> to be 'E'. If
   * <code>serialVersionOnStream</code> is less than 2, initializes
   * <code>locale</code>to the root locale, and initializes If
   * <code>serialVersionOnStream</code> is less than 3, it initializes
   * <code>exponentialSeparator</code> using <code>exponential</code>. Sets
   * <code>serialVersionOnStream</code> back to the maximum allowed value so
   * that default serialization will work properly if this object is streamed
   * out again. Initializes the currency from the intlCurrencySymbol field.
   * 
   * @since JDK 1.1.6
   */
  private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException
  {
    ...
    {
      // didn't have locale; use empty locale
      locale = new Locale("");
    }
  ...
  /**
   * Describes the version of <code>DecimalFormatSymbols</code> present on the
   * stream. Possible values are:
   * <ul>
   * <li><b>0</b> (or uninitialized): versions prior to JDK 1.1.6.
   * 
   * <li><b>1</b>: Versions written by JDK 1.1.6 or later, which include two new
   * fields: <code>monetarySeparator</code> and <code>exponential</code>.
   * <li><b>2</b>: Versions written by J2SE 1.4 or later, which include a new
   * <code>locale</code> field.
   * <li><b>3</b>: Versions written by J2SE 1.6 or later, which include a new
   * <code>exponentialSeparator</code> field.
   * </ul>
